...
TypeRef *Parser::ParseType() {
    ...
    while (true) {
        if (auto t = peek(); t->type != TokenType::MUT && t->type != TokenType::STAR && t->type != TokenType::LBRACK) break;
        ...
        if (peek() && peek()->type == TokenType::STAR) {
            ...
            if (peek() && peek()->type == TokenType::QUESTION) {
                Token q = consume();
        ...
        if (peek() && peek()->type == TokenType::LBRACK) {
            Token lb = consume();
            if (peek() && peek()->type != TokenType::RBRACK) {
